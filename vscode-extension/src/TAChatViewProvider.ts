...
export class TAChatViewProvider implements vscode.WebviewViewProvider {
    public static readonly viewType = 'socratic-ta.chatView';
    private _conversationHistory: {role: string, content: string}[] = [];
    private static _outputChannel: vscode.OutputChannel;

    public static getOutputChannel(): vscode.OutputChannel {
        if (!this._outputChannel) {
            this._outputChannel = vscode.window.createOutputChannel("Socratic TA Logs");
        }
    ...
    public resolveWebviewView(
        ...
        webviewView.webview.onDidReceiveMessage(async (data) => {
            switch (data.type) {
                ...
                case 'startTerminal': {
                    vscode.commands.executeCommand('socratic-ta.startTerminal');
                    break;
                }
                case 'modeChanged': {
...
- Required_Keywords: while, new, delete`;
        ...
        let rule1 = mode === 'Homework Assist'
            ...
            : `1. DIRECT EXPLANATION (STUDY ASSIST): You are in Study Assist mode. Do NOT be Socratic and do NOT end with a question. Provide a direct, concise, and helpful explanation to the student's conceptual question.`;

        // 2. Build the exact system prompt from the synthetic dataset
        const systemPrompt = `[Role]
You are an expert, patient Socratic C++ Teaching Assistant helping a student debug.
...
Raw_Code:
${rawCode}
AST_Metadata: (Hidden for local inference)
[Terminal_Context]
Exit_Code: ${exitCode}
Output:
${terminalOutput}
...
${syllabusMatrix}`;
        ...
        try {
            ...
            // Fallback natively to the raw Docker Bridge IP (192.168.65.254) to bypass Node 18 DNS fetch bugs
            const apiUrl = vscode.workspace.getConfiguration('socraticTa').get('apiUrl') || 'http://192.168.65.254:11434/api/chat';
            
            const requestBody = {
                model: "socratic-ta", // Tagged custom model name
                messages: [
            ...
            const taResponse = data.message?.content || "No response generated.";
...
<body>
    <div class="chat-container">
        <div class="messages" id="messages">
            <div class="message ta-message">Hello! I am your C++ Socratic TA. What are you working on today?</div>
        </div>
        <div>
