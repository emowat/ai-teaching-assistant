...
def _model_matches(
    ...
    *,
    ...
    # Support both the old compressed URL and the new uncompressed prefix
    s3_source = container.get("ModelDataSource", {}).get("S3DataSource", {})
    current_uri = s3_source.get("S3Uri") or container.get("ModelDataUrl")

    return (
        container.get("Image") == image_uri
        and current_uri == model_data_uri
...
def _teardown_autoscaling(session: boto3.Session, cfg: DeployConfig, endpoint_override: str | None = None) -> None:
    """Remove auto scaling policies and CloudWatch alarm for the endpoint variant."""
    asc = cfg.sagemaker.autoscaling
    if not asc.enabled:
        return

    endpoint = endpoint_override or cfg.sagemaker.endpoint_name
...
def invoke(cfg: DeployConfig, prompt: str) -> None:
    ...
    payload = build_async_payload(
        ...
        top_p=smoke.top_p,
        presence_penalty=smoke.presence_penalty,
        frequency_penalty=smoke.frequency_penalty,
...
def status(cfg: DeployConfig, endpoint_override: str | None = None) -> None:
    session = _get_session(cfg)
    sm = session.client("sagemaker")
    endpoint = endpoint_override or cfg.sagemaker.endpoint_name
    try:
...
def cleanup(cfg: DeployConfig, endpoint_override: str | None = None) -> None:
    """Delete endpoint, config, and model — stops all billing for this endpoint."""
    session = _get_session(cfg)
    sm = session.client("sagemaker")
    endpoint = endpoint_override or cfg.sagemaker.endpoint_name

    _teardown_autoscaling(session, cfg, endpoint_override)
...
def main() -> None:
    ...
    parser.add_argument(
        "--endpoint",
        default=None,
        help="Override endpoint name for status and cleanup commands",
    )
    ...
    if args.action == "deploy":
        ...
        status(cfg, args.endpoint)
    elif args.action == "cleanup":
        cleanup(cfg, args.endpoint)
